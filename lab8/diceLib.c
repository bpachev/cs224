...
extern volatile int WDT_debounce_cnt;
extern volatile int switches;
extern volatile int inactivity_cnt;
extern volatile uint8 Die1_last;
extern volatile uint8 Die2_last;
extern volatile int max_roll_frq;
volatile SCREEN_STATE screen = DIE;
...
void doDitty(struct tone *ditty,int ditty_length)
{
	int i=0;
	for(i=0;i<ditty_length;i++){
		register int t_dur = ditty[i].tone_dur;
		doTone(ditty[i].tone_frq,t_dur);
		WDT_delay(t_dur+WDT_1SEC_CNT/5);
	}
}
//------------------------------------------------------------------------------
...
__interrupt void WDT_ISR(void)
{
	...
	if ((switches==0)&&(--inactivity_cnt==0))
	{
		lcd_backlight(OFF);
	}
...
void display_frq_histogram(uint8 *roll_frqs,int frq_length)
{
	int i;
	int height;
	int horizontal_dis;
	...
	lcd_clear();
	lcd_cursor(15,140);
	lcd_printf("Roll frequencies.");
	screen=FRQ;
	for(i=0;i<frq_length;i++)
	{
		height = (60*roll_frqs[i])/max_roll_frq;
		horizontal_dis = 5+i*14;
		lcd_cursor(horizontal_dis,height+47);
		lcd_printf("%d",roll_frqs[i]);
		lcd_cursor(horizontal_dis,35);
		lcd_printf("%d",i+2);
		lcd_rectangle(horizontal_dis,45,5,height,3);
	}
	inactivity_cnt = MAX_IDLE_CNT;
	switches=0;
	_BIS_SR(LPM0_bits+GIE);
...
void init_die_screen(void)
{
	lcd_clear();							// clear LCD
	lcd_backlight(ON);						// turn on LCD backlight
	lcd_wordImage(byu4_image, 42, 110, 1);	// draw byu logo
	lcd_rectangle(10, 45, 60, 60, 3);		// draw double die frame
	lcd_rectangle(90, 45, 60, 60, 3);		// draw double die frame
	if (screen==FRQ)
	{
		drawDie(Die1_last,15,51);
		drawDie(Die2_last,96,51);
