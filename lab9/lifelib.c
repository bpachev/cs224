...
void init_life(enum SEED seed)
{
	...
	switch(seed)
	{
		...
		case BOMB:
		{
			draw_rle_pattern(65, 10, gosper_glider_gun2);
...
/*			draw_rle_pattern(60, 5, pulsar);		// sun
			draw_rle_pattern(31, 2, bird);			// bird
			draw_rle_pattern(40, 20, hwss);			// bigger bird
			draw_rle_pattern(21, 39, glider);		// blows bird apart
			draw_rle_pattern(65, 65, loafer); */
		//	draw_rle_pattern(37,34,block);
		//	draw_rle_pattern(39,67,block);
			draw_rle_pattern(44,50,gosper_glider_gun2);
			//draw_rle_pattern(70,30,jon_arbuckle);
			break;
		}
...
void watchdog_init()
{
	...
	WDT_Sec_Cnt = 16*WDT_1SEC_CNT;			// set WD 1 second counter
...
__interrupt void WDT_ISR(void)
{
	if (--WDT_Sec_Cnt == 0)
	{
		WDT_Sec_Cnt = 16*WDT_1SEC_CNT;		// reset counter
...
const uint8 stab_gosper_glider_gun[] = { "x = 47, y = 14, rule = b3/s23"
